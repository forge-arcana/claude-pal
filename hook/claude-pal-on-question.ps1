# Claude Pal - PreToolUse hook for AskUserQuestion (PowerShell, v2)
...
if ($level -eq 'sound+popup' -or $level -eq 'popup') {
    try {
        ...
        $n.Visible = $true
        $n.ShowBalloonTip(3000, 'Claude Pal', 'Claude is asking you a question.', [System.Windows.Forms.ToolTipIcon]::None)
...
try {
    Set-Content -Path (Join-Path $hooksDir 'claude-pal-signal') -Value "question $(Get-Date -UFormat %s)" -NoNewline
} catch {}
