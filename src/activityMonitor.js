...
function getActivityLevel(usageData = null, sessionData = null) {
    ...
    if (maxPercent >= 90) return 'heavy';
    if (maxPercent >= 75) return 'moderate';
    return 'idle';
}

// Pop culture references for status messages
function getActivityDescription(level, _claudePercent, _tokenPercent) {
    const descriptions = {
        'heavy': {
            short: 'Running low!',
            quirkyOptions: [
...
function getStats(usageData = null, sessionData = null) {
    ...
    return {
        ...
        tokenPercent,
        maxPercent,
        description: getActivityDescription(level, claudePercent, tokenPercent)
    };
}

module.exports = { getActivityLevel, getStats, getActivityDescription };
