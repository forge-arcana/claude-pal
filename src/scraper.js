...
//
// Enable via "claudePal.useLegacyScraper": true in settings.
...
class ClaudeUsageScraper {
    ...
    async acquireBrowserLock() {
        ...
        throw new Error('Browser busy (another Claude Pal instance is using the browser). Please wait and retry.');
    }
                ...
                if (matchesEndpoint(url, API_ENDPOINTS.usage)) {
                    ...
                    if (orgMatch) {
                        ...
                        if (this.currentOrgId && this.currentOrgId !== newOrgId) {
                            console.log(`Claude Pal: Org changed from ${this.currentOrgId} to ${newOrgId}`);
                            if (isDebugEnabled()) {
